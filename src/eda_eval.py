...
setup_logging(log_level='INFO', log_file=Config.LOGS_DIR / 'eda_log.txt')
...
try:
    import spacy
    import scispacy
    _logger.info('Loading scispacy models...')
    nlp_bio = spacy.load("en_ner_bionlp13cg_md")
    nlp_med = spacy.load("en_ner_bc5cdr_md")  
    SCISPACY_AVAILABLE = True
except:
    SCISPACY_AVAILABLE = False
    _logger.warning('''Scispacy not available. Install:
                   pip install spacy'
...
def load_dataset(filepath):
    ...
    _logger.info(f'Loading dataset from {filepath}...')
    df = pd.read_csv(filepath)
    _logger.info(f'Loaded dataset of {len(df)} articles')
    _logger.info(f'Data avaliable: {df.columns.tolist()}')
...
def save_report(filename, report_lines):
    ...
    Args
        file_name (str): Saving location
        report_lines (List(str)): List with report data
    '''
    filepath = Config.EDA_DIR / filename
...
def data_quality_analysis(df):
    ...
    _logger.info('[1] Performing data quality analysis')
    report_lines = []
    
    # missing values
    _logger.info('\t- Missing values check')
    ...
    # empty strings
    _logger.info('\t- Empty strings check')
    ...
    report_lines.append('Empty strings:')
    report_lines.append('\n'.join(empty_total))
    report_lines.append('')
    
    # duplicates
    _logger.info('\t- Duplicates check')
    ...
    # heuristic language
    _logger.info('\t- Language check (latin words > 90% in abstract)')
    def is_english(text):
    ...
    report_lines.append(f'Potentially non-english abstracts: {non_english}')
    report_lines.append('')
    
    save_report('data_quality.txt', report_lines)
...
def descriptive_statistics(df):
    ...
    Returns
        None: Saves text report to 'descriptive_stats.txt'
            Saves visualizations to 'descriptive_stats.png'
            
    '''
    
    _logger.info('[2] Performing descriptive statistics analysis')
    report_lines = []
    
    # text length and ratios
    _logger.info('\t- Text Length Statistics (characters)')
    ...
    _logger.info('\t- Section Ratios (Intro/Full_text, Conclusion/Full_text)')
    report_lines.append('Section Ratios (Intro/Full_text, Conclusion/Full_text)')
    ...
    # years distribution
    _logger.info('\t- Year Distribution')
    ...
    # readability scores
    _logger.info('\t- Calculating readability scores...')
    ...
    plt.tight_layout()
    plt.savefig(Config.EDA_DIR / 'descriptive_stats.png', dpi=300, bbox_inches='tight')
    _logger.info(f'\tPlots saved to {Config.EDA_DIR}/descriptive_stats.png')
    
    save_report('descriptive_stats.txt', report_lines)
...
def lexical_analysis(df):
    ...
    _logger.info('[3] Performing lexical analysis')
    report_lines = []
    ...
    # vocabulary
    _logger.info('\t- Vocabulary statistics')
    ...
    _logger.info('\t- Top 30 Terms (after stopwords)')
    report_lines.append('Top 30 Terms (after stopwords)')
    ...
    _logger.info('\t- TF-IDF Top Terms')
    report_lines.append('TF-IDF Top Terms')
    ...
    # ngrams
    _logger.info('\t- Top N-grams')
    report_lines.append('Top Bigrams')
    ...
    report_lines.append('Top Trigrams')
    ...
    report_lines.append('Top Fourgrams')
    ...
    plt.tight_layout()
    plt.savefig(Config.EDA_DIR / 'lexical_analysis.png', dpi=300, bbox_inches='tight')
    print(f'\tPlots saved to {Config.EDA_DIR}/lexical_analysis.png')
        
    with open(Config.EDA_DIR / 'lexical_analysis.txt', 'w', encoding='utf-8') as f:
        f.write('\n'.join(report_lines))
    _logger.info(f'Report saved to {Config.EDA_DIR}/lexical_analysis.png')
...
def semantic_analysis(df):
    ...
    _logger.info('[4] Performing semantic analysis for intro + conclusion')
    report_lines = []
    ...
    # generate embeddings
    _logger.info(f'\t- Generating embeddings for {len(texts_sample)} texts...')
    _logger.info('\t  Using: sentence-transformers/allenai-specter')
    model = SentenceTransformer('sentence-transformers/allenai-specter')
    embeddings = model.encode(texts_sample, show_progress_bar=True, batch_size=32)
    
    #cosine similarity
    _logger.info('\t- Computing pairwise similarities...')
    report_lines.append('Pairwise cosine similarities')
    
    similarities = cosine_similarity(embeddings) # XXX scaling problem
    avg_similarity = (similarities.sum() - len(similarities)) / (len(similarities) * (len(similarities) - 1))
    report_lines.append(f'\tAverage pairwise similarity: {avg_similarity:.3f}')
    report_lines.append(f'\tStd of similarities: {similarities[np.triu_indices_from(similarities, k=1)].std():.3f}')
    report_lines.append('')
    # umap dimensionality reduction
    _logger.info('\t- UMAP projection...')
    ...
    # topic modelling
    _logger.info('\t BERTopic modeling...')
    ...
    for n in range(5, 15, 2):
        ...
        topics, _ = model.fit_transform(texts_sample)
        score = silhouette_score(embeddings, topics)
        if score > best_score:
            best_score, best_n_topics = score, n
    
    report_lines.append(f'Optimal topics: {best_n_topics} (silhouette: {best_score:.3f})')
    ...
    report_lines.append(f'BERTopics found:{len(set(topics)) - 1}')
    ...
    plt.tight_layout()
    plt.savefig(Config.EDA_DIR / 'semantic_analysis.png', dpi=300, bbox_inches='tight')
    _logger.info(f'\tPlots saved to {Config.EDA_DIR}/semantic_analysis.png')
    
    save_report('semantic_analysis.txt', report_lines)
    
    topic_model.save(str(Config.EDA_DIR / 'bertopic_model'))
    _logger.info(f'BERTopic model saved to {Config.EDA_DIR}/semantic_analysis.png')
...
def pre_rag(df):
    ...
    _logger.info('[5] RAG readiness check')
    report_lines = []
    
    _logger.info('\t- Chunk Size Estimation')
    ...
    report_lines.append(f'Average paragraph length: {np.mean(paragraph_lengths):.0f} chars (±{np.std(paragraph_lengths):.0f})')
    report_lines.append(f'Median paragraph length: {np.median(paragraph_lengths):.0f} chars')
    report_lines.append(f'Average sentences per article: {np.mean(sentence_counts):.0f}')
    
    # chunking
    _logger.info('\t- Simulating Chunking (500 tokens ~2000 chars, overlap 200 chars)')
    chunk_counts = []
    ...
    report_lines.append(f'Average chunks per article: {np.mean(chunk_counts):.1f}')
    report_lines.append(f'Total chunks in corpus: {sum(chunk_counts)}')
    report_lines.append(f'Median chunks per article: {np.median(chunk_counts):.0f}')
    
    # query-document matching
    _logger.info('\t- Query-Document Matching (BM25)')
    ...
    for query in test_queries:
        ...
        report_lines.append(f'\tTop-5 avg BM25 score: {avg_score:.2f}')
        report_lines.append(f'\tBest match score: {scores[top_indices[0]]:.2f}')
    ...
    plt.tight_layout()
    plt.savefig(Config.EDA_DIR / 'pre_rag.png', dpi=300, bbox_inches='tight')
    _logger.info(f'Plots saved to {Config.EDA_DIR}/pre_rag.png')
    
    save_report('pre_rag.txt', report_lines)
...
def extract_entities_bio(texts, nlp_bio):
    ...
    _logger.info(f'\t- Processing {len(texts)} texts with bio model...')
    
    processed_docs = []
    for idx, doc in enumerate(nlp_bio.pipe(texts, batch_size=25, n_process=1), 1):
        processed_docs.append(doc)
        if idx % 25 == 0:
            _logger.info(f'\t  Processed {idx}/{len(texts)}')
...
def extract_entities_med(texts, nlp_med):
    ...
    _logger.info(f'\t- Processing {len(texts)} texts with medical model...')
...
def visualize_network_interactive(G, output_path):
    '''
    ...
    try:
        ...
        _logger.error('PyVis not installed. Run: pip install pyvis')
        return
    ...
    _logger.info(f'Interactive network saved to {output_path}')


def visualize_network_static(G, output_path):
    '''
    ...
    _logger.info(f'Static network saved to {output_path}')
    plt.close()

def compute_cooccurrence_matrix(processed_docs, top_genes):
    '''
    Compute gene/protein co-occurrence matrix from documents.
    
    Args:
        processed_docs (List): List of scispaCy Doc objects with NER annotations
        top_genes (List[str]): List of gene names to analyze
        
    Returns:
        Tuple of (raw_matrix, normalized_matrix):
            - raw_matrix (pd.DataFrame): Original counts for heatmap/reports
            - normalized_matrix (pd.DataFrame): Log-normalized for graph edges
    '''
    ...
    df_normalized = pd.DataFrame(
        np.log1p(matrix_no_diag),  # log(1+x), 8000→9
        index=top_genes, 
        columns=top_genes
    )
    
    _logger.info('\t- Co-occurrence stats:')
    _logger.info(f'\t  Raw max: {df_raw.max().max()} (diagonal)')
    _logger.info(f'\t  Raw max off-diagonal: {matrix_no_diag.max():.0f}')
    _logger.info(f'\t  Normalized max: {df_normalized.max().max():.2f}')
...
def build_entity_network(
    ...
    _logger.info('\t- Adding gene-gene edges...')
    edges_added = 0
    for i, gene1 in enumerate(top_genes):
        for j, gene2 in enumerate(top_genes):
    ...
    _logger.info(f'\t  Added {edges_added} gene-gene edges')
    
    sample_size = min(50, len(texts))
    _logger.info(f'\t- Computing cross-entity edges from {sample_size} documents...')
    ...
    _logger.info(f'\t- Network: {G.number_of_nodes()} nodes, {G.number_of_edges()} edges')
    return G
...
def bio_ner(df: pd.DataFrame):
    ...
    _logger.info('[6] Starting biomedical NER analysis')
    ...
    # 4. Visualizations
    visualize_network_interactive(G, Config.EDA_DIR / 'entity_network.html')
    visualize_network_static(G, Config.EDA_DIR / 'entity_network.png')
    ...
    np.fill_diagonal(cooccur_display.values, 0)
    
    matrix_size = cooccur_display.shape[0]
    if matrix_size > 12:
        sns.heatmap(cooccur_display, annot=False, cmap='YlOrRd',
    ...
    plt.tight_layout()
    plt.savefig(str(Config.EDA_DIR / 'bio_ner.png'), dpi=300, bbox_inches='tight')
    _logger.info(f'Plots saved to {Config.EDA_DIR} / bio_ner.png')
    plt.close()
    ...
    with open(str(Config.EDA_DIR / 'bio_ner.json'), 'w', encoding='utf-8') as f:
        json.dump(results, f, indent=2, ensure_ascii=False)
    _logger.info(f'JSON saved to {Config.EDA_DIR} / bio_ner.json')
    
    # GEXF
    nx.write_gexf(G, str(Config.EDA_DIR / 'entity_network.gexf'))
    _logger.info(f'GEXF saved to {Config.EDA_DIR}/entity_network.gexf')
    ...
    with open(str(Config.EDA_DIR / 'bio_ner_cooccurrence.txt'), 'w', encoding='utf-8') as f:
        f.write('\n'.join(report_lines))
    _logger.info(f'Report saved to {Config.EDA_DIR} / bio_ner_cooccurrence.txt')


def assess_druggability(genes_proteins, df, top_n=30):
    '''
    Assess druggability of genes/proteins based on context keywords.
    
    Scores genes by presence of druggability indicators in surrounding
    text (receptor, kinase, enzyme, etc.). Higher scores indicate
    better drug target potential.
    
    Args:
        genes_proteins (Counter): Gene/protein frequency counter from bio_ner
        
        df (pd.DataFrame): Input DataFrame with 'Full_text' column
        
        top_n (int): Number of top genes to analyze. Defaults to 30.
    
    Returns:
        pd.DataFrame: Druggability scores with columns:
            - gene: Gene/protein name
            - mentions: Total mentions in corpus
            - druggability_score: Weighted score (0-100)
            - druggability_class: 'High', 'Medium', or 'Low'
            - key_indicators: Top keywords found in context
    
    Saves:
        - 'druggability_analysis.png': Visualization
        - 'druggability_analysis.txt': Text report
    '''
    
    _logger.info(f'\t- Assessing druggability for top {top_n} genes/proteins')
    
    druggability_keywords = {
        'high': {
            'receptor': 5,
            'kinase': 5,
            'protease': 4,
    ...
    all_keywords = {}
    for category, kw_dict in druggability_keywords.items():
        all_keywords.update(kw_dict)
    all_keywords.update(therapeutic_keywords)
    
    results = []
    texts = df['Full_text'].fillna('')
    
    for gene, count in genes_proteins.most_common(top_n):
        _logger.info(f'  Analyzing {gene}... ({count} mentions)')
        
        contexts = []
        for text in texts:
            if gene in text:
                sentences = sent_tokenize(text)
                contexts.extend([s for s in sentences if gene in s])
        ...
        sample_contexts = contexts[:min(50, len(contexts))]
        ...
        normalized_score = (score / len(sample_contexts)) * 10 if sample_contexts else 0
        ...
        top_indicators = ', '.join([kw for kw, _ in found_keywords.most_common(3)])
        
        results.append({
            'gene': gene,
            'mentions': count,
            'druggability_score': round(final_score, 1),
            'druggability_class': drugg_class,
            'key_indicators': top_indicators if top_indicators else 'None',
            'contexts_analyzed': len(sample_contexts)
        })
    
    drugg_df = pd.DataFrame(results).sort_values('druggability_score', ascending=False)
    class_counts = drugg_df['druggability_class'].value_counts()
    
    fig, axes = plt.subplots(2, 2, figsize=(16, 12))
    
    top15 = drugg_df.head(15)
    colors = top15['druggability_class'].map({'High': '#2ecc71', 'Medium': '#f39c12', 'Low': '#e74c3c'})
    
    axes[0, 0].barh(range(len(top15)), top15['druggability_score'], color=colors)
    axes[0, 0].set_yticks(range(len(top15)))
    axes[0, 0].set_yticklabels(top15['gene'])
    axes[0, 0].invert_yaxis()
    axes[0, 0].set_xlabel('Druggability Score')
    axes[0, 0].set_title('Top 15 Genes by Druggability', fontsize=14, fontweight='bold')
    axes[0, 0].grid(axis='x', alpha=0.3)
    
    scatter_colors = drugg_df['druggability_class'].map({'High': '#2ecc71', 'Medium': '#f39c12', 'Low': '#e74c3c'})
    axes[0, 1].scatter(drugg_df['mentions'], drugg_df['druggability_score'], 
                      c=scatter_colors, s=100, alpha=0.6, edgecolors='black')
    axes[0, 1].set_xlabel('Mentions in Corpus')
    axes[0, 1].set_ylabel('Druggability Score')
    axes[0, 1].set_title('Druggability vs Frequency', fontsize=14, fontweight='bold')
    ...
    axes[0, 1].legend(handles=legend, title='Druggability')
    
    axes[1, 0].pie(class_counts.values, labels=class_counts.index, autopct='%1.1f%%',
                  colors=['#2ecc71', '#f39c12', '#e74c3c'], startangle=90)
    axes[1, 0].set_title('Druggability Distribution', fontsize=14, fontweight='bold')
    
    top10_genes = drugg_df.head(10)['gene'].tolist()
    keyword_matrix = []
    keywords_list = ['receptor', 'kinase', 'enzyme', 'inhibitor', 'signaling']
    ...
    for gene in top10_genes:
        ...
        for text in texts:
            if gene in text:
                sentences = sent_tokenize(text)
                gene_contexts.extend([s for s in sentences if gene in s])
        
        keyword_counts = []
        for kw in keywords_list:
            count = sum(1 for ctx in gene_contexts[:50] if kw in ctx.lower())
            keyword_counts.append(count)
        keyword_matrix.append(keyword_counts)
    
    sns.heatmap(keyword_matrix, annot=True, fmt='d', cmap='YlGnBu',
                xticklabels=keywords_list, yticklabels=top10_genes,
                ax=axes[1, 1], cbar_kws={'label': 'Mentions'})
    axes[1, 1].set_title('Keyword Context Heatmap (Top 10 Genes)', fontsize=14, fontweight='bold')
    
    plt.tight_layout()
    plt.savefig(Config.EDA_DIR / 'druggability_analysis.png', dpi=300, bbox_inches='tight')
    _logger.info(f'Plots saved to {Config.EDA_DIR}/druggability_analysis.png')
    plt.close()
    
    # CSV
    drugg_df.to_csv(Config.EDA_DIR / 'druggability_scores.csv', index=False)
    _logger.info(f'CSV saved to {Config.EDA_DIR}/druggability_scores.csv')
    
    # Text report
    report_lines = []
    report_lines.append(f'\nAnalyzed {len(drugg_df)} genes/proteins')
    report_lines.append(f'Contexts analyzed: {drugg_df["contexts_analyzed"].sum()} sentences')
    report_lines.append('\nTop 20 Drug Targets:')
    report_lines.append(f"{'Gene':<20} {'Score':<8} {'Class':<10} {'Mentions':<10} {'Key Indicators'}")
    
    for idx, row in drugg_df.head(20).iterrows():
        report_lines.append(
            f"{row['gene']:<20} {row['druggability_score']:<8.1f} "
            f"{row['druggability_class']:<10} {row['mentions']:<10} {row['key_indicators']}"
        )
    
    report_lines.append('\n\nDruggability Class Distribution:')
    for cls, cnt in class_counts.items():
        report_lines.append(f'  {cls:<10}: {cnt:3d} genes ({cnt/len(drugg_df)*100:5.1f}%)')
    
    save_report('druggability.txt', report_lines)
    
    return drugg_df

def pathway_mechanism_analysis(genes_proteins, df, top_n=20):
    '''
    Analyze interaction mechanisms between genes/proteins.
    
    Uses dependency parsing and relation keywords to extract
    interaction types (inhibits, activates, binds, regulates).
    Builds directed graph of biological pathways.
    ...
    Args:
        ...
        top_n (int): Number of top genes to analyze. Defaults to 20.
    
    Returns:
        nx.DiGraph: Directed graph with interaction types as edge labels
    
    Saves:
        - 'pathway_network.png': Directed network visualization
        - 'pathway_analysis.txt': Interaction catalog
        - 'pathway_network.gexf': Gephi-compatible graph file
    '''
    
    _logger.info(f'\t- Analyzing pathways/mechanisms for top {top_n} genes')
    
    relation_patterns = {
        'inhibits': ['inhibit', 'suppress', 'block', 'prevent', 'antagonize'],
        'activates': ['activate', 'stimulate', 'promote', 'enhance', 'induce'],
        'binds': ['bind', 'interact', 'associate', 'complex'],
        'regulates': ['regulate', 'modulate', 'control', 'influence'],
        'phosphorylates': ['phosphorylate', 'dephosphorylate'],
        'cleaves': ['cleave', 'process', 'degrade', 'proteolys'],
        'upregulates': ['upregulate', 'increase', 'elevate'],
        'downregulates': ['downregulate', 'decrease', 'reduce']
    }
    
    G = nx.DiGraph()
    
    top_genes = [g for g, _ in genes_proteins.most_common(top_n)]
    texts = df['Full_text'].fillna('')
    
    for gene in top_genes:
        G.add_node(gene, count=genes_proteins[gene])
    
    interactions = []
    
    _logger.info('Extracting gene-gene interactions...')
    ...
    for text in texts:
        ...
        for sentence in sentences:
            ...
            genes_in_sentence = [g for g in top_genes if g in sentence]
            ...
            for relation_type, keywords in relation_patterns.items():
                for keyword in keywords:
                    if keyword in sentence_lower:
                        for i, gene1 in enumerate(genes_in_sentence):
                            for gene2 in genes_in_sentence[i+1:]:
                                gene1_pos = sentence.find(gene1)
                                gene2_pos = sentence.find(gene2)
                                keyword_pos = sentence_lower.find(keyword)
                                
                                if gene1_pos < keyword_pos < gene2_pos:
                                    interactions.append({
                                        'source': gene1,
                                        'target': gene2,
                                    ...
                                    if G.has_edge(gene1, gene2):
                                        ...
                                        G.add_edge(gene1, gene2, 
                                                  ...
                                                  weight=1,
                                                  primary_relation=relation_type)

    relation_counts = Counter([i['relation'] for i in interactions])
    ...
    if G.number_of_edges() > 0:
        ...
        node_sizes = [G.nodes[n].get('count', 10) * 50 for n in G.nodes()]
        
        nx.draw_networkx_nodes(G, pos, node_size=node_sizes, 
                              node_color='lightblue', alpha=0.8, ax=axes[0])
        ...
        relation_color_map = {
            ...
            'binds': '#3498db',
            'regulates': '#9b59b6',
            'phosphorylates': '#f39c12',
            'cleaves': '#e67e22',
            'upregulates': '#1abc9c',
            'downregulates': '#34495e'
        }
        
        for u, v, data in G.edges(data=True):
            primary_rel = data.get('primary_relation', 'regulates')
            edge_colors.append(relation_color_map.get(primary_rel, '#95a5a6'))
            edge_widths.append(min(data.get('weight', 1) * 1.5, 5))
        ...
        # Legend
        legend_elements = [Patch(facecolor=color, label=rel.title()) 
                          for rel, color in relation_color_map.items()]
        axes[0].legend(handles=legend_elements, loc='upper left', fontsize=9)
        axes[0].set_title('Pathway Network (Directed)', fontsize=14, fontweight='bold')
        axes[0].axis('off')
    else:
        axes[0].text(0.5, 0.5, 'No interactions found', ha='center', va='center')
        axes[0].axis('off')
    
    if relation_counts:
        relations, counts = zip(*relation_counts.most_common())
        colors_bar = [relation_color_map.get(r, '#95a5a6') for r in relations]
        
        axes[1].barh(range(len(relations)), counts, color=colors_bar)
        axes[1].set_yticks(range(len(relations)))
        axes[1].set_yticklabels([r.title() for r in relations])
        axes[1].invert_yaxis()
        axes[1].set_xlabel('Number of Interactions')
        axes[1].set_title('Interaction Types Distribution', fontsize=14, fontweight='bold')
        axes[1].grid(axis='x', alpha=0.3)
    
    plt.tight_layout()
    plt.savefig(Config.EDA_DIR / 'pathway_network.png', dpi=300, bbox_inches='tight')
    _logger.info(f'\tPlot saved to {Config.EDA_DIR}/pathway_network.png')
    plt.close()

    report_lines = []

    report_lines.append(f'\nAnalyzed {len(top_genes)} genes')
    report_lines.append(f'Found {len(interactions)} interactions')
    report_lines.append(f'Network edges: {G.number_of_edges()}')
    
    report_lines.append('\n\nInteraction Types:')
    for rel, count in relation_counts.most_common():
        report_lines.append(f'  {rel.title():<20}: {count:3d} instances')
    
    report_lines.append('\n\nKey Interactions (Sample):')
    for interaction in interactions[:30]:
        report_lines.append(
            f"{interaction['source']} --[{interaction['relation']}]--> {interaction['target']}"
        )
        report_lines.append(f"  Context: {interaction['sentence']}")
        report_lines.append('')
    
    save_report('pathway_mechanisms.txt', report_lines)
    
    G_export = G.copy()
    for u, v, data in G_export.edges(data=True):
        for key, value in list(data.items()):
            if isinstance(value, (list, dict)):
                # Конвертируем списки/словари в JSON-строки
                data[key] = json.dumps(value) if value else ""
            elif not isinstance(value, (str, int, float, bool)):
                # Удаляем неподдерживаемые типы
                del data[key]
    
    nx.write_gexf(G_export, str(Config.EDA_DIR / 'pathway_network.gexf'))
    
    _logger.info(f'GEXF saved to {Config.EDA_DIR}/pathway_network.gexf')
    
    interactions_json = {
        'nodes': [{'id': n, 'mentions': G.nodes[n].get('count', 0)} for n in G.nodes()],
        'edges': [
            {
                'source': u,
                'target': v,
        ...
        'statistics': dict(relation_counts)
    }
    
    with open(Config.EDA_DIR / 'pathway_network.json', 'w', encoding='utf-8') as f:
        json.dump(interactions_json, f, indent=2)
    _logger.info(f'JSON saved to {Config.EDA_DIR}/pathway_network.json')
...
def mesh_terms_analysis(df):
    ...
    '''
    _logger.info('MeSH terms analysis')
    report_lines = []
    ...
    _logger.info(f'Total unique MeSH terms: {len(mesh_freq)}')
    _logger.info(f'Total MeSH annotations: {sum(mesh_freq.values())}')
    _logger.info('Top 30 MeSH Terms:')
    
    for term, count in mesh_freq.most_common(30):
        pct = count / len(df) * 100
        _logger.info(f'{term[:50]:50s}: {count:3d} ({pct:5.1f}%)')
    ...
    for cat, count in sorted(category_counts.items(), key=lambda x: x[1], reverse=True):
        if count > 0:
            _logger.info(f'{cat:20s}: {count:3d} terms')
    ...
    plt.tight_layout()
    plt.savefig(Config.EDA_DIR / 'mesh_terms.png', dpi=300, bbox_inches='tight')
    _logger.info(f'Plot saved to {Config.EDA_DIR}/mesh_terms.png')
    ...
    with open(Config.EDA_DIR / 'mesh_terms.txt', 'w', encoding='utf-8') as f:
        f.write('\n'.join(report_lines))
    _logger.info(f'Report saved to {Config.EDA_DIR}/mesh_terms.txt')
...
def citation_density_analysis(df):
    ...
    '''
    _logger.info('Citation density analysis')
    ...
    _logger.info('Citation Statistics:')
    _logger.info(f'\tAverage citations per article: {avg_citations:.1f}')
    _logger.info(f'\tMedian citations per article: {median_citations:.0f}')
    _logger.info(f'\tAverage citation density: {avg_density:.2f} per 1000 chars')
    _logger.info(f'\tMax citations in single article: {df_valid["citation_count"].max():.0f}')
    
    _logger.info('Most Heavily Cited Articles:')
    ...
    for idx, row in top_cited.iterrows():
        _logger.info(f'\t{row["Title"][:60]:60s} - {row["citation_count"]:.0f} citations ({row["citations_per_1k_chars"]:.1f}/1k)')
    ...
    plt.tight_layout()
    plt.savefig(Config.EDA_DIR / 'citation_density.png', dpi=300, bbox_inches='tight')
    _logger.info(f'Plot saved to {Config.EDA_DIR}/citation_density.png')
    ...
    with open(Config.EDA_DIR / 'publication_metrics.txt', 'w', encoding='utf-8') as f:
        f.write('\n'.join(report_lines))
    _logger.info(f'Metrics TXT saved to {Config.EDA_DIR}/publication_metrics.txt')

if __name__ == '__main__':
    df = load_dataset(Config.CORPUS_CSV)
    ...
    if SCISPACY_AVAILABLE:
        ...
        genes_proteins, _, _ = extract_entities_bio(
            df['Full_text'].fillna('').tolist(),
            nlp_bio
        )
        
        drugg_df = assess_druggability(genes_proteins, df, top_n=30)
        pathway_graph = pathway_mechanism_analysis(genes_proteins, df, top_n=20)
